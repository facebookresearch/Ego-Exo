...
def train_epoch(
    ...
    if cfg.MODEL.FREEZE_STAGES > 0 or cfg.MODEL.FREEZE_BN:
        for name, m in model.named_modules():
            if isinstance(m, torch.nn.BatchNorm3d) and name.startswith("module.s") and \
                ((int(name[8]) <= cfg.MODEL.FREEZE_STAGES and name[9] == ".") or cfg.MODEL.FREEZE_BN):
                m.eval()
