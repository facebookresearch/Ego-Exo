...
def inflate_weight(state_dict_2d, state_dict_3d):
    ...
    for k, v2d in state_dict_2d.items():
        #assert k in state_dict_3d.keys(), f"{k}"
        if k not in state_dict_3d.keys():
            logger.warn(f"{k} not in state_dict_3d")
            continue
        v3d = state_dict_3d[k]
...
def load_checkpoint(
    ...
    logger.info("Loading checkpoing {}".format(path_to_checkpoint))
    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            for key in keys:
                if 'headobjhead' in key:
                    newkey = key.replace('headobjhead', 'handobjhead')
                    checkpoint["model_state"][newkey] = checkpoint["model_state"][key]
            ms.load_state_dict(checkpoint["model_state"], strict=False)
            noinit = []
            for key in ms.state_dict():
                if key not in checkpoint["model_state"]:
                    noinit.append(key)
            logger.info(f"not init: {noinit}")
            extra = []
            for key in checkpoint["model_state"].keys():
                if key not in ms.state_dict():
                    extra.append(key)
...
def load_test_checkpoint(cfg, model):
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH != "":
        ...
        if cfg.TEST.LOAD_BEST:
            last_checkpoint = os.path.join(
                get_checkpoint_dir(cfg.OUTPUT_DIR), "checkpoint_best.pyth"
            )
        else:
...
def load_train_checkpoint(cfg, model, optimizer):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            ...
            model,
            cfg.NUM_GPUS > 1,
            optimizer if cfg.TRAIN.RESUME else None,
            inflation=cfg.TRAIN.CHECKPOINT_INFLATE,
            convert_from_caffe2=cfg.TRAIN.CHECKPOINT_TYPE == "caffe2",
        )
        # start_epoch = checkpoint_epoch + 1
        start_epoch = checkpoint_epoch + 1 if cfg.TRAIN.RESUME else 0
    else:
        start_epoch = 0
