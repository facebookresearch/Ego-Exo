...
def torchvision_decode(
    ...
    return v_frames, video_meta["video_fps"], decode_all_video, start_idx
...
def pyav_decode(
    ...
    assert frames_length == duration, f"{frames_length}, {duration}"

    if duration is None:
        # If failed to fetch the decoding information, decode the entire video.
        decode_all_video = True
        video_start_pts, video_end_pts = 0, math.inf
        start_idx, end_idx = 0, frames_length - 1
    else:
        ...
        # print(video_start_pts, video_end_pts, timebase)
    ...
    # print(f"decode s {start_idx} e {end_idx} length {frames.shape[0]}")

    return frames, fps, decode_all_video, start_idx
...
def decode(
    ...
    try:
        if backend == "pyav":
            frames, fps, decode_all_video, _start_idx = pyav_decode(
                container,
                sampling_rate,
                num_frames,
                clip_idx,
            ...
            frames, fps, decode_all_video, _start_idx = torchvision_decode(
                container,
                sampling_rate,
                num_frames,
                clip_idx,
    ...
    #print(f"_startidx {_start_idx} s {start_idx} e {end_idx}")
    # Perform temporal sampling from the decoded video.
    frames, index = temporal_sampling(frames, start_idx, end_idx, num_frames)
    # print(f"index {index}")
    if ret_index:
        index = [int(_start_idx) + i for i in index]
        return frames, index
    else:
        return frames
